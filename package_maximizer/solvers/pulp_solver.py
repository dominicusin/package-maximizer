...
class PulPSolver(ConstraintSolver):
    ...
    def solve(self, packages: Iterable[Package]) -> list[str]:
        ...
        x = LpVariable.dicts("Package", [pkg.name for pkg in package_list], cat=LpBinary)
        ...
        for pkg in package_list:
            for conflict in pkg.conflicts:
                if conflict in x:
                    ...
                    prob += x[pkg.name] + x[conflict] <= 1, f"Conflict_{pkg.name}_{conflict}"
    ...
    def solve_with_weights(
        ...
        prob += lpSum([x[pkg.name] * weights.get(pkg.name, 1.0) for pkg in package_list]), "Total_Weight"
