...
class PackageMaximizer:
    ...
    def __init__(
        ...
        self.manager = (
            manager
            if isinstance(manager, PackageManagerType)
            else PackageManagerType(manager)
        )
        ...
        self._cache_manager: CacheManager | None = (
            CacheManager(default_ttl=cache_ttl) if use_cache else None
        )
    ...
    def solve_with_weights(
        self, packages: Sequence[Package], weights: dict[str, float] | None = None
    ) -> list[str]:
        """
    ...
    def _get_cache_key(self, method: str, *args, **kwargs) -> str:
        ...
        for arg in args:
            if isinstance(arg, (list, tuple)):
                key_parts.append(
                    str([hashlib.md5(str(item).encode()).hexdigest() for item in arg])
                )
            else:
    ...
    def check_constraints(
        ...
        return {
            c.package: c.satisfied_by(versions.get(c.package, "")) for c in constraints
        }

    def analyze(
        self, installed: list[str] | None = None, proposed: list[str] | None = None
    ) -> dict[str, Any]:
        """
